...
function deletePost(){
	Swal.fire({
		  ...
		  if (result.isConfirmed) {
			  ...
			  vform.append($('<input/>', {type:'hidden', name:'no', value:${galleryDetail.gno } }));
			  vform.appendTo('body');
			  vform.submit();
	...
	<section class="page-section" id="gallery">
		<div class="container">
			<div class="text-center">
				<h2 class="section-heading text-uppercase">갤러뤼상쉐보기</h2>
			</div>
			<div class="card mb-4" style="min-height: 500px;">
				<div class="card-body">
					<div class="h2">${galleryDetail.gtitle }</div>
					<div class="row p-2 bg-secondary">
						<div class="col align-middle text-start">
						${galleryDetail.mname }
							<c:if test="${galleryDetail.mid eq sessionScope.mid }">
							*본인*
							<img alt="edit" src="./img/edit.png">
							<img alt="delete" src="./img/delete.png" title="글삭제" onclick="deletePost(${galleryDetail.gno })">
							</c:if>
						</div>
						<div class="col align-middle text-end">${galleryDetail.gdate }</div>
					</div>
					<img alt="" class="card-img-top" src="./upfile/${galleryDetail.gfile }" >
					<div class="mt-4 h-auto">${galleryDetail.gcontent }</div>
				</div>
			</div>
